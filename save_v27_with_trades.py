...
def save_equity_curves():
    ...
    for period in ['train', 'test', 'holdout']:
        ...
        """, (
            ...
            'V27',
            period,
            f'FIFO金字塔, SC=5, SL=3%, mcl=3, 下根K线开盘价成交',
            f'V27: SC=5, SL=3%, mcl=3',
            '{"sell_confirm_bars":5,"max_consecutive_losses":3,"stop_loss_pct":3.0}'
        ))
        ...
        conn.commit()
        print(f'V27 {period}: equity_bars={len(result_rows)}, trades={len(trade_markers)}, initial={result_df["equity"].iloc[0]:.0f}, final={result_df["equity"].iloc[-1]:.0f}, exp_id={exp_id}')
