...
f = open('/tmp/grid_results.log', 'w')
def log(msg):
    f.write(msg + '\n')
    f.flush()

log('Starting quick grid')
...
log('')
log('Grid (SC=0 only, testing SL):')

results = []
sl_values = [0.0, 0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 4.0, 5.0]
row = 'SC=0   '
for sl in sl_values:
    all_eq = []
    for tok in tokens:
        res, _ = run_backtest(all_sigs[tok], initial_capital_usd=3000.0, fee_bps=5.0, position_size_pct=0.01, max_position_pct=0.90, min_order_notional=0.0, fill_probability=0.85, seed=42, max_consecutive_losses=0, single_loss_stop_pct=0.0, sell_vol_pct=0.05, stop_loss_pct=sl, sell_confirm_bars=0)
    ...
    results.append((0, sl, total_ret, mdd))
    log(f'  SL={sl}% done')
...
for r in sorted(results, key=lambda x: x[2], reverse=True)[:3]:
    log(f'  SL={r[1]}% => Return={r[2]:.2f}%, MDD={r[3]:.2f}%')
