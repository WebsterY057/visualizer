...
from backtest_v26_next_open import add_signals, run_backtest, DATA_DIR, PERIODS, TOKENS_11
import numpy as np
import sqlite3
import pandas as pd
...
ID_MAP = {'train': 27276, 'test': 27277, 'holdout': 27278}
...
def save_equity_curves():
    ...
    for period in ['train', 'test', 'holdout']:
        ...
        """, (
            ...
            f'V26_corrected_{period}',
            'V26',
            period,
            f'FIFO金字塔, SC=3, SL=3%, mcl=3, 下根K线开盘价成交 (正确参数)',
            f'V26: SC=3(正确传递), SL=3%, mcl=3, 其余同V26',
            '{"vwap_sustain":1,"vwap_type":"rolling","vwap_window":60,"sell_confirm_bars":3,"max_consecutive_losses":3,"stop_loss_pct":3.0}'
        ))
        ...
        for tok in TOKENS_11:
            ...
            res, trd = run_backtest(sig,
                ...
                max_consecutive_losses=3, single_loss_stop_pct=0.0,
                sell_vol_pct=0.05, stop_loss_pct=3.0,
                sell_confirm_bars=3)
        ...
        conn.commit()
        print(f'V26_corrected {period}: {len(result_rows)} bars, initial={result_df["equity"].iloc[0]:.0f}, final={result_df["equity"].iloc[-1]:.0f}, exp_id={exp_id}')

    conn.close()
    print('V26_corrected Done!')
