...
f.write('signal generated\n')
f.flush()

res, trd = run_backtest(sig, initial_capital_usd=3000.0, fee_bps=5.0, position_size_pct=0.01, max_position_pct=0.90, min_order_notional=0.0, fill_probability=0.85, seed=42, max_consecutive_losses=0, single_loss_stop_pct=0.0, sell_vol_pct=0.05, stop_loss_pct=0.0, sell_confirm_bars=0)
f.write(f'backtest done, ret={(res["equity"].values[-1]/3000-1)*100:.1f}%\n')
f.flush()
f.close()