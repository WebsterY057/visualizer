...
ID_MAP = {'train': 27270, 'test': 27271, 'holdout': 27272}
...
def save_equity_curves():
    ...
    for period in ['train', 'test', 'holdout']:
        ...
        conn.commit()
        print(f'V28 {period}: {len(result_rows)} minute bars saved, initial={result_df["equity"].iloc[0]:.0f}, final={result_df["equity"].iloc[-1]:.0f}')
