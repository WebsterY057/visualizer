...
ID_MAP = {'train': 27264, 'test': 27265, 'holdout': 27266}
...
def save_equity_curves():
    ...
    for period in ['train', 'test', 'holdout']:
        ...
        conn.commit()
        print(f'V26 {period}: {len(result_rows)} minute bars saved, initial={result_df["equity"].iloc[0]:.0f}, final={result_df["equity"].iloc[-1]:.0f}, exp_id={exp_id}')
