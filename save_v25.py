...
ID_MAP = {'train': 27282, 'test': 27283, 'holdout': 27284}
...
def save_equity_curves():
    ...
    for period in ['train', 'test', 'holdout']:
        ...
        """, (
            ...
            'V25',
            period,
            f'FIFO金字塔, SC=0, SL=3%, mcl=3, 下根K线开盘价成交',
            f'V25: SC=0, SL=3%, mcl=3, 其余同V26',
            '{"vwap_sustain":1,"vwap_type":"rolling","vwap_window":60,"sell_confirm_bars":0,"max_consecutive_losses":3,"stop_loss_pct":3.0}'
        ))
        ...
        conn.commit()
        print(f'V25 {period}: {len(result_rows)} bars, initial={result_df["equity"].iloc[0]:.0f}, final={result_df["equity"].iloc[-1]:.0f}, exp_id={exp_id}')
