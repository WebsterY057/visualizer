...
def save_equity_curves():
    ...
    for period in ['train', 'test', 'holdout']:
        ...
        conn.commit()
        print(f'V28 {period}: equity_bars={len(result_rows)}, trades={len(trade_markers)}, initial={result_df["equity"].iloc[0]:.0f}, final={result_df["equity"].iloc[-1]:.0f}, exp_id={exp_id}')
