...
from backtest_v27_next_open import add_signals, run_backtest, DATA_DIR, PERIODS, TOKENS_11
import numpy as np
import sqlite3
import pandas as pd
...
ID_MAP = {'train': 27279, 'test': 27280, 'holdout': 27281}
...
def save_equity_curves():
    ...
    for period in ['train', 'test', 'holdout']:
        ...
        """, (
            ...
            f'V27_corrected_{period}',
            'V27',
            period,
            f'FIFO金字塔, SC=5, SL=3%, mcl=3, 下根K线开盘价成交 (正确参数)',
            f'V27: SC=5(正确传递), SL=3%, mcl=3, 其余同V27',
            '{"vwap_sustain":1,"vwap_type":"rolling","vwap_window":60,"sell_confirm_bars":5,"max_consecutive_losses":3,"stop_loss_pct":3.0}'
        ))
        ...
        conn.commit()
        print(f'V27_corrected {period}: {len(result_rows)} bars, initial={result_df["equity"].iloc[0]:.0f}, final={result_df["equity"].iloc[-1]:.0f}, exp_id={exp_id}')

    conn.close()
    print('V27_corrected Done!')
