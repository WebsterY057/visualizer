    ...
    for period in ['train', 'test', 'holdout']:
        ...
        """, (
            ...
            json.dumps({
                ...
                'sell_vol_pct': 0.05, 'stop_loss_pct': 0.0, 'sell_confirm_bars': 5
            })
        ))
        ...
        combined = pd.concat(list(all_token_equity.values()), ignore_index=True).sort_values('minute').reset_index(drop=True)

        all_minutes = sorted(combined['minute'].unique())
        minute_idx = pd.DatetimeIndex(all_minutes)

        portfolio_equity = pd.Series(0.0, index=minute_idx, dtype=float)
        for tok in all_token_equity:
            tok_data = all_token_equity[tok].sort_values('minute').set_index('minute')['equity']
            tok_data = tok_data.reindex(minute_idx).ffill().fillna(3000.0)
            portfolio_equity = portfolio_equity + tok_data.values

        result_df = pd.DataFrame({'minute': minute_idx, 'equity': portfolio_equity.values})
        ...
        for bar_index, (_, row) in enumerate(result_df.iterrows()):
            ...
            portfolio_rows.append((
                ...
                str(row['minute']), row['equity'], 0, 0, 0, drawdown
            ))
        ...
        cur.executemany('''INSERT INTO trade_markers
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)''', all_trade_markers)

        conn.commit()
        print(f'V23 {period}: portfolio={len(portfolio_rows)} bars, trades={len(all_trade_markers)}')
